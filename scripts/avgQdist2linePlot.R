...
# run: avgQdist2linePlot.R <fastx_toolbox_result_..._all.txt> <dest.folder>
#
# Stéphane Plaisance - VIB-BITS - 2010-07-08 v2.2
# small edits 2014-07-31

args <- commandArgs()
myfile <- args[6]
mypath <- args[7]

name<-sub( ".txt*$", "", basename(myfile) )
...
plot(data$A_Count~data$column,
	...
	xlim=c(1,readlen),
	ylim=c(min,max),
	main=paste("normalized base frequency from", name, sep="\n"))
